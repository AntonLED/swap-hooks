...
CSV of its own numbers beside the PDF. Figures carry no in-figure titles (the
LaTeX captions do that) and share one style defined in `experiments/figures.py`:
STIX serif, closed pgfplots-style frame, dark blue = beats the baseline / brick
red = loses / grey = interval covers zero, filled marker = the 95% bootstrap
...
| `99-figure-sandbox.ipynb`    | `<run>/summary.csv`, per-swap traces | nothing until `WRITE = True`                       |

`99-figure-sandbox.ipynb` is the interactive styling loop: every paper figure in
its own editable cell (style knobs at the top, ipympl live rendering with
zoom/pan), slow data prep cached in separate cells. The grid and storm-trend
cells carry a `RELATIVE` toggle that normalises by the panel's median baseline
result. Once a style change is final, mirror it into the `draw_*.py` script —
the scripts stay the source of record and are what write the CSVs.
...
| `draw_seed_forest.py`        | seed robustness of the storm corner (matrix vs five seed replicates) |
| `draw_fee_behaviour.py`      | per-policy directional fee series through one stormy day             |
| `draw_demand_calibration.py` | one measured intraday shape at the five κ heights                    |
...
uv run python analysis/draw_seed_forest.py   # needs results/sensitivity/ runs
```
